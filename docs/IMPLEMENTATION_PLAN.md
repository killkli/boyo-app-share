...
| 階段 | 狀態 | 開始日期 | 完成日期 | 完成度 |
|------|------|----------|----------|--------|
| Stage 1: 專案初始化與基礎建設 | ⏳ Not Started | - | - | 0% |
...
#### 2.5 登入 API (TDD)
- [ ] **TDD - 登入測試**: `tests/integration/api/auth/login.test.ts`
- [ ] 實作 `server/api/auth/login.post.ts`
- [ ] 測試密碼驗證
- [ ] 測試 JWT 生成
...
#### 2.9 認證頁面
- [ ] 建立 `pages/login.vue`
- [ ] 建立 `pages/register.vue`
- [ ] 建立 `middleware/auth.ts` (前端路由保護)
- [ ] 建立 `middleware/guest.ts` (未登入才能訪問)

#### 2.10 Layout 整合
- [ ] 更新 `layouts/default.vue` 顯示使用者狀態
- [ ] 加入登出按鈕
- [ ] 加入使用者選單
